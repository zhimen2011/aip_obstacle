...
# --- 内容驱动解析的正则与常量 ---
_BRG_DIST_PATTERN = re.compile(r"(?<![A-Za-z0-9])(\d{3})/(\d{3,6})(?![A-Za-z0-9])")
...
def _clean_cells(cells: List[str]) -> List[str]:
    return [str(c).strip() if c else "" for c in cells]


def _id_only_row(cells: List[str]) -> Optional[tuple[int, str]]:
    non_empty = [
        (i, str(c).strip())
    ...
    idx, value = non_empty[0]
    if _IS_DIGITS.match(value):
        return idx, value
    return None


def _row_obstacle_id(cells: List[str]) -> str:
...
def _name_cell_index(cells: List[str]) -> Optional[int]:
    _, _, bd_idx = _find_brg_dist(cells)
    end = bd_idx if bd_idx >= 0 else len(cells)

    for i in range(0, end):
        text = str(cells[i]).strip() if cells[i] else ""
        if not text:
            continue
        if _IS_PURE_NUMBER.match(text):
            continue
        if _BRG_DIST_PATTERN.search(text):
            continue
        if _LAT_PATTERN.search(text) or _LON_PATTERN.search(text):
            continue
        return i
    return None


def _is_split_data_row(cells: List[str]) -> bool:
    if _is_data_row(cells):
        return False

    mag_brg, _, _ = _find_brg_dist(cells)
    if mag_brg is None:
        return False

    return _name_cell_index(cells) is not None


def _attach_id_to_split_row(row: dict, obstacle_id: str) -> dict:
    cells = list(row["cells"])
    idx = _name_cell_index(cells)
    if idx is None:
        idx = 0

    existing = cells[idx].strip() if cells[idx] else ""
    cells[idx] = f"{existing}\n{obstacle_id}" if existing else obstacle_id
...
def _merge_rows(group: List[dict]) -> List[dict]:
    """合并被拆分的行。续行拼回上一行对应列。"""
    merged: List[dict] = []
    pending_split: Optional[dict] = None
    for row in group:
        cells = _clean_cells(row["cells"])
        # 优先排除章节标题/表头/备注等非数据行
        if _is_skip_row(cells):
            pending_split = None
            continue

        id_only = _id_only_row(cells)
        if pending_split is not None:
            if id_only:
                merged.append(_attach_id_to_split_row(pending_split, id_only[1]))
                pending_split = None
                continue
            pending_split = None
        ...
        elif _is_split_data_row(cells):
            pending_split = {
                **row,
                "cells": cells,
            }
        elif id_only:
            # pdfplumber sometimes emits a duplicate id-only row after a full row.
            # Keep it from being appended to the previous obstacle record.
            if merged and _row_obstacle_id(merged[-1]["cells"]) == id_only[1]:
                continue

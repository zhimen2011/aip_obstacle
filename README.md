...
- 与更大航空运行系统的对接接口
- 历史版本比对（AIP 周期性更新）
## 2026-06-15 更新：PDF 表格兜底与质量校验

- PDF 解析现在采用“表格解析为主、文本层解析兜底”的流程：表格结果正常时保留表格值；表格失败或字段明显异常时，按同一障碍物编号使用文本层结果补齐。
- 方位/距离识别已收紧，避免把 `RWY01/36L/36R` 这类跑道字符串误识别为 `01/36`。
- `confidence_score` 现在用于业务质量提示：缺磁方位、缺距离、缺海拔、距离明显过小或磁方位排序异常会降为高风险；缺经纬度不再降低可信度，也不会触发黄/红色提示。
- 中国 AIP 障碍物按 15km 内、15km 外分别以磁方位递增排列；系统会按这个规则检查明显跳序的记录。
